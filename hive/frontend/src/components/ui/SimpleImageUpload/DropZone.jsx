...
const DropZone = ({
      ...
      onDrop: (acceptedFiles, rejectedFiles) => {
        acceptedFiles.map((file) => {
          ...
          img.onload = () => {
            if (imgHeight && img.height !== imgHeight) {
            ...
            } else if (imgWidth && img.width !== imgWidth) {
              ...
              parentCallback(handleOnClose(acceptedFiles));
            }
          };
        });
  ...
  // pass the accepted image file to parent
  const handleOnClose = (acceptedFiles) => {
    return uploadType
      ? { uploadType: uploadType, acceptedFiles: acceptedFiles }
      : acceptedFiles;
  };
    ...
    <div className="text-center mt-5">
        <section className="container">
          <div {...getRootProps({ style })}>
            <input {...getInputProps()} />
            <p className="block text-slate-600 dark:text-slate-300 text-sm font-normal">Drag and drop an image here or click to select an image.</p>
            {(helpMessage && helpMessage!="")? <span className="block text-slate-500 dark:text-slate-300 text-xs font-normal">{helpMessage}</span>:<></>}
          </div>
          <aside style={thumbsContainer}>{isImage ? imgThumb : thumbs}</aside>
        </section>
    </div>
  );
};
